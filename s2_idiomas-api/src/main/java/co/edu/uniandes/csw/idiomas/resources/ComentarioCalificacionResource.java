...
public class ComentarioCalificacionResource {
    ...
    private static final Logger LOGGER = Logger.getLogger(ComentarioCalificacionResource.class.getName());
     ...
     @Inject
    private ComentarioCalificacionLogic logica;
     ...
     */
    @POST
    public ComentarioCalificacionDTO createComment(ComentarioCalificacionDTO comentario) throws BusinessLogicException 
    {
        LOGGER.log(Level.INFO, "commentResource createcomment: input: {0}", comentario);
        // Convierte el DTO (json) en un objeto Entity para ser manejado por la lógica.
        ComentarioCalificacionEntity comentarioEntity = comentario.toEntity();
        // Invoca la lógica para crear un comentario nuevo
        ComentarioCalificacionEntity comentarioCalifEntity = logica.createCalifComment(comentarioEntity);
        // Como debe retornar un DTO (json) se invoca el constructor del DTO con argumento el entity nuevo
        ComentarioCalificacionDTO comentarioDTO = new ComentarioCalificacionDTO(comentarioCalifEntity);
    ...
    public ComentarioCalificacionDTO getcomment(@PathParam("commentsId") Long commentId)
    {
        LOGGER.log(Level.INFO, "ComentarioResource getComment: input: {0}", commentId);
        ComentarioCalificacionEntity comentarioEntity = logica.getComment(commentId);
        if (comentarioEntity == null) {
            throw new WebApplicationException("El recurso /comments/" + commentId + " no existe.", 404);
        }
        ComentarioCalificacionDTO detailDTO = new ComentarioCalificacionDTO(comentarioEntity);
    ...
    public void deletecomment(@PathParam("commentsId") Long commentId) throws BusinessLogicException {
        ...
        if (logica.getComment(commentId) == null) {
            throw new WebApplicationException("El recurso /comments/" + commentId + " no existe.", 404);
        }
    ...
    @PUT
    @Path("{commentsId: \\d+}")
    public ComentarioCalificacionDTO updatecomment(@PathParam("commentsId") Long commentId, ComentarioCalificacionDTO comentario) 
    {
        ...
        ComentarioCalificacionDTO detailDTO = new ComentarioCalificacionDTO(logica.updateComment(commentId, comentario.toEntity()));
