...
public class ComentarioActividadResource {
    ...
    @Inject
    private ComentarioActividadLogic logica;
     ...
     */
    @POST
    public ComentarioActividadDTO createComment(ComentarioActividadDTO comentario) throws BusinessLogicException 
    {
        LOGGER.log(Level.INFO, "commentResource createcomment: input: {0}", comentario);
        // Convierte el DTO (json) en un objeto Entity para ser manejado por la lógica.
        ComentarioActividadEntity comentarioEntity = comentario.toEntity();
        // Invoca la lógica para crear un comentario nuevo
        ComentarioActividadEntity comentarioCalifEntity = logica.createActivityComment(comentarioEntity);
        // Como debe retornar un DTO (json) se invoca el constructor del DTO con argumento el entity nuevo
        ComentarioActividadDTO comentarioDTO = new ComentarioActividadDTO(comentarioCalifEntity);
    ...
    public ComentarioActividadDTO getcomment(@PathParam("commentsId") Long commentId)
    {
        LOGGER.log(Level.INFO, "ComentarioResource getComment: input: {0}", commentId);
        ComentarioActividadEntity comentarioEntity = logica.getComentario(commentId);
        if (comentarioEntity == null) {
            throw new WebApplicationException("El recurso /comments/" + commentId + " no existe.", 404);
        }
        ComentarioActividadDTO detailDTO = new ComentarioActividadDTO(comentarioEntity);
    ...
    @PUT
    @Path("{commentsId: \\d+}")
    public ComentarioActividadDTO updatecomment(@PathParam("commentsId") Long commentId, ComentarioActividadDTO comentario) 
    {
        ...
        ComentarioActividadDTO detailDTO = new ComentarioActividadDTO(logica.updateComment(commentId, comentario.toEntity()));
