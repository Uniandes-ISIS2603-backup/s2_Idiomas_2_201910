...
public class ComentarioActividadLogic {
    ...
    private static final Logger LOGGER = Logger.getLogger(ComentarioActividadLogic.class.getName());
...
//        Verifica que el texto no sea mayor a 300 caracteres.
    ...
    public ComentarioActividadEntity getComentario(Long commentId) 
    {
        LOGGER.log(Level.INFO, "Inicia proceso de consultar la chat con id = {0}", commentId);
        // Note que, por medio de la inyección de dependencias se llama al método "find(id)" que se encuentra en la persistencia.
        ComentarioActividadEntity comentarioEntity = persistence.find(commentId);
     ...
     */
    public ComentarioActividadEntity updateComment(Long  commentId, ComentarioActividadEntity comentarioEntity) 
    {
        LOGGER.log(Level.INFO, "Inicia proceso de actualizar la chat con id = {0}",  commentId);
        // Note que, por medio de la inyección de dependencias se llama al método "update(entity)" que se encuentra en la persistencia.
        ComentarioActividadEntity newEntity = persistence.update(comentarioEntity);
